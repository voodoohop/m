...
var evalStreamEntry = function(loadedSequences, newSequence) {
  ...
  if (unsatisfiedImports.count() > 0) {
    ...
    console.log("existing exports".bold, loadedSequences.entrySeq().map(s => ({
      name: s[0],
...
export var processedAndReEval = new Bacon.Bus();
...
var processedSequences_noStack = evaluated //.filter((n) => n.get("evaluated"))//.flatMap((n) => n.get("evaluated").toJS());
  .flatMap(n => {
    ...
    if (!n.exports)
      return Bacon.never();
    return Bacon.fromArray(n.exports.map(seqName => _.extend({
