...
var mGenerator = function(generatorProducer, name, curryArgCount = 0, toStringOverride=null) {
  ...
  var genProducer = function(...args) {
    ...
    res[wu.iteratorSymbol] = () => {
      productionStats[name] = productionStats[name] ? productionStats[name]+1 : 1;

      if (Math.random()<0.00001)
        console.log(productionStats);
      // generatorProducer.setLoopElements = function(numElements) {
      //
      // };
      return generatorProducer(...args);
    }
    if (toStringOverride)
...
var MLoopData = mGenerator( function*(dataNode) {
  for (var data of dataNode) {
    ...
    var noneIterable = true; var noneInfinite = true;
    var dataLengths=[];
    for (let k of keys) {
      // console.log(""+data[k]);
    ...
    if (noneIterable) {
      // console.log("noneiterable",data);
      yield* getIterator(MLoop(dataNode));
      return;
    }
    // if (noneInfinite) {
    //   var nextVals = data.map( => undefined);
    //   var iterators = data.map( => null);
    //   while (true) {
    //
    //     // if (iterators == null)
    //     _.values(nextVals).filter(v => v == undefined) .(data).map(getIterator);
    //     var nextVals = iterators.map((i) => i.next().value);
    //     if ()
    //     yield
    //  }
    // }
    ...
    for (var props of MZip(..._.values(data))) {
      ...
      //resData._data = data;
    //  console.log("datanode",dataNode, "datainst",data,"props",props, "result", resData);
    //  console.trace();
      yield resData;
    }
  }
...
var makeChainable = function (lib,name,funcToChain) {
  return function(...args) {
    ...
    _.each(lib,(origFunction, origFunctionName) => {
      ...
      //    console.log("made chainable", name);
      //    console.trace();
      //  }
    });
    return result;
