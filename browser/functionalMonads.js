...
import {prettyToString,toStringObject,addFuncProp, clone, addObjectProp, isIterable} from "./utils";
...
//  let name = "unnamed";
  ...
  genProducer.isIterableProducer = true;
...
var MNotePlayer = mGenerator(function*(node) {
  ...
  for (let me of node) {
    ...
    yield {time: me.time, play:playMethod};
  }
...
var MAutomatePlay = mGenerator(function*(name,node) {
  for (let v of node) {
    ...
    yield {time: v.time, play:playMethod}
  }
},"automatePlay",2);
...
var MEvent = mGenerator(function*(props=false) {
  if (props)
    yield* MEventProperties(props,MEvent());
  else
    yield Object.freeze({});
},"evt");
...
var MEventProperties = mGenerator(function*(props,node) {
    ...
    if (keys.length == 0) {
      ...
      var val = imProps[key];
      delete imProps[key];
      yield* MEventProperties(imProps, MProperty(key, val, node));
    }
},"set",2);
...
//    console.log("convertToObject",convertToObject(mapFunc(e)));
    ...
    for (let res of mapped)
      yield* MEventProperties(convertToObject(res),MEvent(e));
  }
...
var MPluck = mGenerator(function*(propertyName, node) { yield* MMapOp(e => e[propertyName], node)},"pluck",2);

var MMapWithMemory = mGenerator(function*(initial, mapFunc,node) {
  let current = initial;
  yield* MEventProperties(convertToObject(current), MEvent(e));
  for (let e of node) {
...
var MRepeat = mGenerator(function*(n, node) { yield* MTake(n, MLoop(node));},"repeat",2);
...
var MBranch = mGenerator(function*(condition, branchNode, elseNode, node) {
    for (let e of node) {
      ...
      yield* branchTo
        ...
        .map((n) => n.set({time: n.time+e.time})
        );
    }
},"branch",4);
...
var MSequenceEndMarker = mGenerator(function* () { yield* MEvent({type: "endMarker" })},"endMarker");
...
var MMetronome = mGenerator(function*(tickDuration,node) { yield* MTime( MCount(0, tickDuration), MCompose(node,MSequenceEndMarker()));},"metronome");
...
var MTimeFromDurations = mGenerator(function*(node)   {
  ...
  yield* MTime(durationSumIterator, MCompose(node, MSequenceEndMarker()));
},"timeFromDurations");
...
//    addFunction("note",MNoteEvent);
    ...
    addFunction("notePlay",MNotePlayer,false);
