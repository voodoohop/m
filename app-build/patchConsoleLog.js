...
var toStringForLog = (function(a) {
  ...
  if ((typeof a === 'undefined' ? 'undefined' : $traceurRuntime.typeof(a)) == "object" && a.isTom)
    return toStringForLog("" + a);
  if (depth > 0 && (typeof a === 'undefined' ? 'undefined' : $traceurRuntime.typeof(a)) == "object" && !util.isArray(a)) {
    return "{ " + Object.keys(a).map((function(key) {
      return key.bold + ": " + toStringForLog(a[$traceurRuntime.toProperty(key)], 0);
    })).join(", ") + " }";
  }
  ...
  return function() {
    ...
    if (bgCol)
      mapped = mapped.map((function(a) {
        return a[$traceurRuntime.toProperty(bgCol)];
      }));
