(function() {
  var colors, commandClass, commandClasses, commands, config, fs, getGitVersion, getPythonVersion, name, npm, optimist, parseOptions, path, printVersions, setupTempDirectory, spawn, wordwrap, _, _i, _j, _len, _len1, _ref, _ref1;
  ...
  printVersions = function(args, callback) {
    ...
    return getPythonVersion(function(pythonVersion) {
      return getGitVersion(function(gitVersion) {
  ...
  getGitVersion = function(callback) {
    var npmOptions;
    npmOptions = {
      userconfig: config.getUserConfigPath(),
      globalconfig: config.getGlobalConfigPath()
    ...
    return npm.load(npmOptions, function() {
      var git, outputChunks, spawned, _ref2;
      git = (_ref2 = npm.config.get('git')) != null ? _ref2 : 'git';
      ...
      spawned.on('error', function() {});
      return spawned.on('close', function(code) {
        var gitName, version, versionName, _ref3;
        if (code === 0) {
          _ref3 = Buffer.concat(outputChunks).toString().split(' '), gitName = _ref3[0], versionName = _ref3[1], version = _ref3[2];
          version = version != null ? version.trim() : void 0;
        }
        return callback(version);
      });
  ...
  module.exports = {
    run: function(args, callback) {
      var Command, callbackCalled, command, options;
