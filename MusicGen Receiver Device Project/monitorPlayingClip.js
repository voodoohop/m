...
log("Reload:", new Date);
   ...
   function playingClipChanged(param) {
	if (param[0] === "playing_slot_index") {
       var playingClip = param[1];
		post("clip changed",param[0],param[1],"\n");  
	    if (playingClip >=0) {
	 		clip.path= "this_device canonical_parent clip_slots "+playingClip+" clip";
  			var clip_content = clip_content(clip);     	
      		post("\nplayingclip", clip_content.name);
  			var oscOut = "/playingClipNotes";
  			var json = JSON.stringify(clip_content);
  			log(json.length);
  			outlet(0,[oscOut,LZString.compressToBase64(json)]);
   		}
	}
   }
   var api = new LiveAPI(playingClipChanged);
